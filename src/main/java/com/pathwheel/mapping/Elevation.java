...
public class Elevation {
	...
	public static List<Double> fromOpenElevationApi(List<GeographicCoordinate> coordinates) throws Exception {
		...
		System.out.println("fromOpenElevationApi: "+request.toString());
		//System.exit(0);
		...
		System.out.println("fromOpenElevationApi2 ");
		//connection.connect();
		try( DataOutputStream wr = new DataOutputStream( connection.getOutputStream())) {
			System.out.println("fromOpenElevationApi3");
			   wr.write( request.toString().getBytes(StandardCharsets.UTF_8) );
			System.out.println("fromOpenElevationApi4");
		}

		BufferedReader br = new BufferedReader(new InputStreamReader(connection.getInputStream()));
		String buffer = "";
		String s = "";
		System.out.println("fromOpenElevationApi5");
		while (null != ((s = br.readLine()))) {
			 buffer = buffer + s;			
		}
		br.close();
		System.out.println("fromOpenElevationApi6");
