...
import com.pathwheel.model.TravelMode;
...
public class JdbcPavementSampleDAO extends JdbcDataAccessObject implements PavementSampleDAO {
	...
	public void register(PavementSample data, String smartDevice) {
		...
		try	{
			...
			sql = "INSERT INTO pathwheel.pavement_sample(latitude_init, longitude_init, latitude_end, longitude_end, elapsed_time, vertical_acceleration, speed, distance, accuracy, pavement_type_id, user_id, smart_device_id, steps, travel_mode_id) "
		    ...
		    for(Double verticalAcceleration : data.getVerticalAccelerations()) {
				//perguntar para leonardo sobre as tabelas de dados extras que ele criou (pavement_test_data_va e pavement_test_data)
			    sql = "INSERT INTO pathwheel.pavement_sample_va(pavement_sample_id, vertical_acceleration) VALUES (?, ?);";
			    stmt = conn.prepareStatement(sql);			    
			    i= 1;
	...
	public List<PavementSample> fetchByArea(List<GeographicCoordinate> verticesPolygon, int travelModeId) {
		...
		try	{
			...
			String sql = "SELECT ps.id, ps.latitude_init, ps.longitude_init, ps.latitude_end, ps.longitude_end, "+
					"ps.elapsed_time, ps.vertical_acceleration, ps.speed, ps.distance, ps.accuracy, "+
					"ps.pavement_type_id "+
					"FROM pathwheel.pavement_sample ps "+
					"LEFT join pathwheel.pavement_type pt ON pt.id = ps.pavement_type_id "+
					"WHERE ps.registration_date BETWEEN localtimestamp - cast('1 year' as interval) AND localtimestamp "+
					"AND ps.travel_mode_id = ? "+
					"AND public.ST_Intersects( "+
					"public.ST_GeographyFromText('LINESTRING('||ps.latitude_init||' '||ps.longitude_init||','||ps.latitude_end||' '||ps.longitude_end||')'), "+
					"public.ST_GeographyFromText('POLYGON(("+polygon+"))') "+
					") AND ps.exclusion_date is null "+
					"LIMIT 3000;";
